...
def test_render_end_to_end(article, backend, tmp_path):
    ...
    result = pipeline.render(str(article), out, options_for("article"), backend)
...
def test_pauses_lengthen_the_output(article, backend, tmp_path):
    ...
    quiet = pipeline.render(str(article), tmp_path / "a.mp3", options_for("article"), backend)
    original = chunk_mod.PAUSE_PARAGRAPH
    try:
        chunk_mod.PAUSE_PARAGRAPH = original * 4
        louder = pipeline.render(str(article), tmp_path / "b.mp3", options_for("article"), backend)
    finally:
